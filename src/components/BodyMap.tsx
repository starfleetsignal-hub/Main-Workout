...
// One-off photo reference in place of the vector figure, for muscles where
// we have a suitable illustration on hand. Everything else keeps the
// generic SVG body map.
const PHOTO_OVERRIDES: Record<string, ReturnType<typeof require>> = {
  chest: require('../../assets/muscles/chest-front.jpg'),
};

export function BodyMap({ diagram, color, muscleId }: { diagram: Diagram; color: string; muscleId: string }) {
  const photo = PHOTO_OVERRIDES[muscleId];
